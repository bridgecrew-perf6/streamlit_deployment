...
st.title('StackOverflow 2021 Developer Survey')
...
df = get_data(50)
df.rename(columns={'Employment': 'Selection'}, inplace=True)
...
if st.checkbox('Select to see top 10 databases and frameworks used'):
    ...
    st.write(pd.DataFrame(grp_data.groupby(['DatabaseHaveWorkedWith'], sort=False)['Count'].count().rename_axis(["DatabaseHaveWorkedWith"]).nlargest(10)))
    st.subheader('Top 10 Frameworks used')
    st.write(pd.DataFrame(grp_data.groupby(['WebframeHaveWorkedWith'], sort=False)['Count'].count().rename_axis(["WebframeHaveWorkedWith"]).nlargest(10)))
...
st.subheader('DevType')
devtype = selected_segmented_data.groupby('DevType')['Selection'].count()
st.bar_chart(devtype)
...
st.subheader('Yearly Compensation')
ConvertedCompYearly = selected_segmented_data.groupby('ConvertedCompYearly')['Selection'].count()
st.bar_chart(ConvertedCompYearly)
